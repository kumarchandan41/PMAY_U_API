...
namespace WeeklyReportAPI.Controllers
{
    ...
    public class WeeklyReportController : ApiController
    {
            ...
            if (httpRequest.Files.Count > 0)
            {
                ...
                try
                {
                    ...
                    for (int i = 1; i < value.Rows.Count; i++)//value.Rows.Count - value.Rows.Count  9310
                    {
                        ...
                        //else
                        //{
                           // obj.Project_Name = value.Rows[i][11].ToString().Substring(0,100);
                        //}20-01-2014 00:00:00
                        
                        string strDt1 = value.Rows[i][12].ToString().Substring(10);
        ...
        //InsertDistt_MasterData
        [HttpPost]
        [Route("InsertDistt_MasterData")]
        public string InsertDistrictData(Distt_Master data)
        {
            string message = "";
            if (data != null)
            {
                EmployeeEntities objEntity = new EmployeeEntities();
                objEntity.Distt_Master.Add(data);
                int i = objEntity.SaveChanges();
                if (i > 0)
                {
                    message = "Distt Master data saved succeffully";
                }
                else
                {
                    message = "Save failed";
